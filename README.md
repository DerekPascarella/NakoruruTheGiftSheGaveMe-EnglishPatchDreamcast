...
<ul>
 ...
 <li>Voice-acted dialogue audio persists across multiple dialogue boxes as to not be prematurely halted when English text consumes more than a single dialogue box (read more in <a href="https://www.dreamcast-talk.com/forum/viewtopic.php?f=52&t=14736&start=30#p163650">this Dreamcast-Talk.com forum post</a>).</li>
 ...
 <li>Rewrote and optimized code in the functions responsible for pushing icon data to VMU LCD, thus eliminating an issue causing intermittent ignored input (i.e., button presses) on the <a href="http://www.dreammods.net/dreamconn.html">DreamConn+</a> controller (read more in <a href="https://twitter.com/DerekPascarella/status/1598008862736384001">this Twitter thread</a> and <a href="https://www.dreamcast-talk.com/forum/viewtopic.php?f=52&t=14736&p=171379#p171379">this Dreamcast-Talk.com forum post</a>).</li>
 ...
 <li>Support has been added for PAL console users leveraging RGB SCART, where originally RGB support only functioned when using NTSC-U and NTSC-J consoles.</li>
...
<img align="right" src="https://i.imgur.com/PbVgeuJ.jpg" width="180">Throughout the development of this translation patch, testing was performed across the spectrum of both emulators and ODEs, as well as optical disc.  Interestingly, this game (both in its original form and in its patched form) has one very mild issue when played via some emulators and ODEs, causing a texture flicker in between some scene transitions.  When this occurs, the on-screen character(s) will flicker slightly before fading away.  This is caused by faster data-read speeds than developers originally intended (or tested against) when played via GD-ROM.
<br><br>
Please understand that this texture flicker occurs in the original, unaltered version of the game too, and was in no way introduced by the translation patch.  It's not a game-breaking issue, but instead strictly cosmetic.  Furthermore, those playing this game on an optical disc (i.e., a CDI burned to a CD-R) will not experience it.
<br><br>
The solution to said problem is to artificially limit the speed at which data is read from the disc image in order to more closely mimic the performance of a GD-ROM drive.  While achievable on ODEs, no such a setting has been found in the <a href="https://github.com/flyinghead/flycast">Flycast</a>, <a href="http://demul.emulation64.com/">Demul</a>, <a href="https://segaretro.org/NullDC">NullDC</a>, or <a href="https://gitlab.com/simulant/community/lxdream-nitro">LXDream Nitro</a> emulators used during development and testing.  However, <a href="https://redream.io/">redream</a> seems to behave true to original hardware by default.
<br><br>
Below are the configurations needed to impose the required data-read limit on applicable ODEs.
<br><br>
<ul>
<li><b>GDEMU</b></li>
Two options exist for users of authentic GDEMUs, or the more recent clones that can be upgraded.
<br><br>
Preferrably, use firmware <a href="https://gdemu.wordpress.com/firmware/updating-gdemu/">version 5.20.5</a>, where Deunan updated the list of games requiring optimal timings to include "Nakoruru: The Gift She Gave Me".  This version also includes the same fix for mild texture stuttering in another English translation patch, "<a href="https://github.com/DerekPascarella/SakuraWarsColumns2-EnglishPatchDreamcast">Sakura Wars: Columns 2</a>".
<br><br>
Alternatively, users not willing to upgrade to 5.20.5 (but still using version 5.20 or newer) can impose a global read-limit by setting the <tt>read_limit</tt> parameter to <tt>1250</tt> in the <tt>GDEMU.ini</tt> configuration file in the root of their SD card (<a href="https://github.com/DerekPascarella/NakoruruTheGiftSheGaveMe-EnglishPatchDreamcast/blob/main/ode_configs/GDEMU.ini">see example configuration file here</a>).  More can be read about the <tt>read_limit</tt> option on the <a href="https://gdemu.wordpress.com/operation/gdemu-operation/">GDEMU Operation</a> page.  It's important to mention that globally limiting speed at which data is read from the SD card will cause incompatibility with some Atomiswave conversions, hence the recommendation to use firmware version 5.20.5.
<br><br>
...
<br><br>
<ul>
<li>A 100% fully-unlocked save file, granting players access to all of the game's built-in extra content (accessible from the "EXTRAS" option in the game's main menu). Note that this doesn't affect story progress, as this is a system save file, separate from story progress save files.</li>
