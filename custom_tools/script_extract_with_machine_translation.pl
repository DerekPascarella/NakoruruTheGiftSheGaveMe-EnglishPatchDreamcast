	...
	if(scalar(@script_file_lines) > 0)
	{
		...
		for(my $j = 0; $j < scalar(@script_file_lines); $j ++)
		{
			...
			elsif($j > 0 && $script_file_lines[$j - 1] =~ /^$previous_line_number\|/)
			{
				...
				my $post_data = uri_encode("auth_key=XXXXXXXXXXXXXXXXXXXX&target_lang=EN-US&source_lang=JA&text=" . $japanese_text);
			...
			else
			{
				...
				my $post_data_x = uri_encode("auth_key=361bf0c8-9eaa-dcc4-d610-d59c8324f6a4:fx&target_lang=EN-US&source_lang=JA&text=" . $japanese_text_x);
